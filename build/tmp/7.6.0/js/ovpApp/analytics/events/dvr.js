...
(function () {
    ...
    dvrEvents.$inject = ["$rootScope", "analyticsService", "$log", "analyticsAssetHelper"];
    angular.module('ovpApp.analytics.events.dvr', ['ovpApp.analytics.analyticsService']).factory('analyticsDvr', dvrEvents).run(["analyticsDvr", function loadHandler(analyticsDvr) {
        return analyticsDvr;
    }]);
    ...
    function dvrEvents($rootScope, analyticsService, $log, analyticsAssetHelper) {
        ...
        function cdvrRequestToRecord(asset, action, settings, errorData) {
            try {
                ...
                if (settings) {
                    data.recordingOptions = angular.extend(data.recordingOptions, analyticsAssetHelper.convertAllValuesToStrings(settings));
                }
        ...
        function attachEventListeners() {
            try {
                $rootScope.$on('update-dvr', function (evt, schedule, asset, action) {
                    if (action && action.actionType === 'cdvrScheduleRecording') {
                        cdvrRequestToRecord(asset, action);
                    } else if (action && action.actionType === 'cdvrDeleteRecording') {
                        cdvrRequestToDelete(asset, action);
                    } else if (action && (action.actionType === 'cdvrCancelRecording' || action.actionType === 'cdvrCancelSeriesRecording')) {
                ...
                $rootScope.$on('Analytics:cdvr-record-failed', function (evt, asset, action, settings, errorData) {
                    cdvrRequestToRecord(asset, action, settings, errorData);
                });
                $rootScope.$on('Analytics:cdvr-delete-recording-failed', function (evt, asset, action, error) {
                    cdvrRequestToDelete(asset, action, error);
                });
                $rootScope.$on('Analytics:cdvr-cancel-recording-failed', function (evt, asset, action, settings, error) {
                    cdvrRequestToCancel(asset, action, settings, error);
                });
                $rootScope.$on('Analytics:cdvr-schedule-series-recording', function (evt, asset, action, settings) {
        ...
        return {
            cdvrRequestToRecord: cdvrRequestToRecord,
            cdvrRequestToDelete: cdvrRequestToDelete
        };
    }
})();
//# sourceMappingURL=../../../maps-babel/ovpApp/analytics/events/dvr.js.map

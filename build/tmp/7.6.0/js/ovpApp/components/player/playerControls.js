...
(function () {
    ...
    playerControlTimer.$inject = ["$timeout", "$rootScope", "config"];
    PlayerControlsController.$inject = ["$rootScope", "$scope", "version", "$timeout", "alert", "TWCVideoJS", "playerStreamService", "profileService", "messages", "$state", "$window", "ondemandGoBack", "globalKeydown", "keyMap", "BookmarkService", "dummyEANAsset", "config", "CDVR_STATE", "ovpFullscreen", "playerControlTimer", "playerService", "playerErrors", "locationService", "$q", "blockingScreenService", "errorCodesService"];
    angular.module('ovpApp.playerControls', ['ovpApp.player.blockingScreen', 'ovpApp.product.service', 'ovpApp.components.ovp.rating', 'ovpApp.config', 'ovpApp.playerControls.miniGuide', 'ovpApp.playerControls.miniGuideData', 'ovpApp.playerControls.skipButton', 'ovpApp.player.streamService', 'ovpApp.services.locationService', 'ovpApp.services.parentalControlsService', 'ovpApp.directives.draggable', 'ovpApp.messages', 'ovpApp.dataDelegate', 'cfp.hotkeys', 'dibari.angular-ellipsis', 'ovpApp.directives.focus', 'ovpApp.services.dateFormat', 'rx', 'ovpApp.services.rxUtils', 'selectn', 'ovpApp.directives.ovp-fullscreen', 'ovpApp.playerControls.ovpScrubber', 'ovpApp.player', 'ovpApp.product.productActionService', 'ovpApp.services.profileService', 'ovpApp.components.modal', 'ovpApp.components.ovp.ccSettings', 'ovpApp.services.errorCodes', 'ovpApp.services.stbService', 'ovpApp.services.flickTo', 'ovpApp.services.windowFocus', 'ovpApp.components.ovp.remotePlayerCCSettings', 'ajoslin.promise-tracker', 'ovpApp.player.sapService', 'ovpApp.directives.keydown', 'ovpApp.ondemand.goback']).directive('playerControls', PlayerControls).directive('preventDefault', preventDefault).factory('playerControlTimer', playerControlTimer);
    ...
    /* @ngInject */
    function PlayerControlsController($rootScope, $scope, version, $timeout, alert, TWCVideoJS, playerStreamService, profileService, messages, $state, $window, ondemandGoBack, globalKeydown, keyMap, BookmarkService, dummyEANAsset, config, CDVR_STATE, ovpFullscreen, playerControlTimer, playerService, playerErrors, locationService, $q, blockingScreenService, errorCodesService) {
        var vm = this;
        ...
        var goBack = function goBack() {
            return ondemandGoBack(vm.asset, vm.isTrailer);
        };
        ...
        var scrubberReadyDefer = $q.defer();
        ...
        function registerBlockingListeners() {
            var blockingScreenSubscription = undefined;

            blockingScreenSubscription = blockingScreenService.getSource().subscribe(function (_ref) {
            ...
            $scope.$on('player:parentalControlsUnblocked', function () {
                vm.pcBlockedMessage = '';
            });

            $scope.$on('player:showBlockingScreen', function () {
                vm.pcBlockedMessage = 'This show is blocked by parental controls.' + ' Press unblock button to play the video';
            });
        ...
        function activate() {
            ...
            $scope.$on('seek-to-position', function (event, data) {
                vm.seekToPosition(data.positionSec, data.sourceElement);
            });

            $scope.$on('player-control:hide', function () {
                if (ovpFullscreen.isEnabled()) {
                    vm.controllerBarVisible = false; // hide if in full screen mode
                }
            });

            $scope.$on('player-control:guide-toggled', function (evt, data) {
                if (ovpFullscreen.isEnabled()) {
                    vm.controllerBarVisible = data.isVisible;
                }
            });

            $scope.$on('player-control:click', function () {
                return onPlayerMouseDown();
            });

            var unsubscribe = playerService.assetLoadedSource.subscribe(function (options) {
            ...
            var keydownHandler = globalKeydown.observable
            // Ignore key events if not playing video and vod mode
            .filter(function (event) {
                return vm.isValidPlayRoute() && vm.isVodMode() && !vm.slider.options.isAdPlaying && !(event.ctrlKey || event.shiftKey || event.altKey || event.metaKey);
        ...
        function getEptTime() {
            if (vm.isTrailer || vm.isStartOver) {
                ...
                var bookmark = vm.asset.bookmark;
                if (bookmark) {
        ...
        function getStartTime() {
            if (vm.isTrailer || vm.isStartOver) {
                ...
                var bookmark = vm.asset.bookmark;
                if (!bookmark || bookmark.complete || bookmark.playMarkerSeconds >= bookmark.runtimeSeconds) {
                    return 0;
                } else {
        ...
        function setScrubbingDisabled() {
            vm.scrubbingDisabled = vm.stream && vm.stream.streamProperties.tricks_mode && vm.stream.streamProperties.tricks_mode.FASTFORWARD !== undefined && !(vm.adBreaks && vm.adBreaks.length);
        }
        ...
        function calculateEpt(playbackTimestamp) {
            var adBreaksTime = 0;
            vm.adBreaks.find(function (adBreak) {
        ...
        function onAdBreaksCreated(event) {
            if (isVodMode()) {
                ...
                if (event && event.source) {
                    event.source.forEach(function (adBreak) {
        ...
        function onPlaybackStarted() {
            ...
            if (!vm.isVodMode()) {
                ...
                scrubberReadyDefer.promise.then(function () {
                    return vm.controllerBarVisible = true;
                });
            }
        }
        ...
        function onPlayerPositionChanged(event) {
            ...
            var newPos = parseInt(event.PlaybackTimestamp / 1000);
        ...
        function seekToPosition(positionSec, sourceElement) {
            ...
            var newPosSec = Math.max(0, Math.min(positionSec, vm.bufferLenMsec / 1000 - 5));
        ...
        function prepareToSetBookmark(event, postRightAway) {
            if (isVodMode() && !event.Blocked && !vm.isTrailer) {
                BookmarkService.setBookmark(postRightAway, Math.floor(event.PlaybackTimestamp / 1000), Math.floor(calculateEpt(event.PlaybackTimestamp) / 1000), Math.floor(event.Runtime / 1000));
            }
        }
        ...
        function onPlayerError(error) {
            if (isVodMode()) {
                (function () {
                    ...
                    var codes = TWCVideoJS.FlashVideoPlayer.playerErrorCodes;
                    var keys = Object.keys(codes);
                    if (keys.find(function (k) {
                        return codes[k] === error.errorID;
                    })) {
        ...
        function deregisterCallbacks(playerEvents) {
            for (var eventName in playerEvents) {
        ...
        function registerCallbacks() {
            ...
            for (var eventName in playerEvents) {
                if (playerEvents.hasOwnProperty(eventName)) {
                    (function () {
                        var rawFunction = playerEvents[eventName];
                        playerEvents[eventName] = function () {
                            rawFunction.apply(undefined, arguments);
                            $scope.$evalAsync();
                        };
            ...
            var stopVodListener = $rootScope.$on('stop-vod', function () {
                deregisterCallbacks(playerEvents);
            });
        ...
        function ariaLoadingMessage() {
            ...
            if (vm.asset != dummyEANAsset && isLiveMode()) {
                var preface = message ? message : '';
                var episodeTitle = vm.asset.episodeTitle ? vm.asset.episodeTitle : '';
                var title = vm.asset.title ? vm.asset.title : '';
        ...
        function isVodMode() {
            return vm.controls === vm.CONTROLS_MODE.VOD || vm.controls === vm.CONTROLS_MODE.CDVR;
        }
...
//# sourceMappingURL=../../../maps-babel/ovpApp/components/player/playerControls.js.map

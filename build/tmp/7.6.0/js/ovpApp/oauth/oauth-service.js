...
(function () {
     ...
     */
    OauthService.$inject = ["$q", "$http", "$log", "$cookies", "$timeout", "OauthDataManager", "oAuthConfig", "httpUtil", "config", "OauthKeyMap", "messages", "$rootScope", "OauthSigner", "$injector", "streamPlusService", "profileService", "ovpStorage", "storageKeys", "$window", "$location", "errorCodesService"];
    angular.module('ovpApp.oauth').provider('OauthService', OauthProvider);
    ...
    function OauthProvider() {
        ...
        return {
            $get: ["$injector", function $get($injector) {
                if (!oService) {
    ...
    function OauthService($q, $http, $log, $cookies, $timeout, OauthDataManager, oAuthConfig, httpUtil, config, OauthKeyMap, messages, $rootScope, OauthSigner, $injector, streamPlusService, profileService, ovpStorage, storageKeys, $window, $location, errorCodesService) {
        var service = this;
        ...
        var _isAuthenticated = false,
            ...
            _oauthError = null,
            loginDefer = null,
            _refreshExpireBufferTime = 30000,
            //Add this when determining if the token is expired
        ...
        function isAuthenticated() {
            ...
            return updateToken(force).then(function () {
        ...
        function createOAuthSessionFromMasqueradeToken(masqueradeToken, temp) {
            if (shouldAttemptMasquerade()) {
                ...
                return $http({
                    ...
                    var masqResponse = getOAuthDataFromResponse(response);
        ...
        function shouldRejectAutoOauth() {
            var autoAuthSignOutTime = ovpStorage.getItem(storageKeys.autoAuthSignOutTime);
            return autoAuthSignOutTime && Date.now() - autoAuthSignOutTime < _autoOauthDisabledTimeInMilliseconds;
        }
        ...
        function waitUntilAuthenticated() {
            return isAuthenticated().then(function (authenticated) {
                if (!authenticated) {
                    if (!loginDefer) {
                        loginDefer = $q.defer();
                    }
        ...
        function updateToken() {
            ...
            if (!_isAuthenticated && !_sentResumeAuth && !isExpired(oAuthData.expiration, _refreshExpireBufferTime) && !_sentAutoAccess) {
        ...
        function userLogin(username, password) {
            ...
            return getTemporaryToken().then(function (temp) {
                return authorizeUser(username, password, temp).then(function (verifier) {
        ...
        function refreshToken() {
            if (!_refreshTokenMutex) {
                _refreshTokenMutex = getToken()['catch'](function (tokenErr) {
                    return getTemporaryToken().then(function (tempToken) {
                        return refreshTokenWithDeviceVerifier(tempToken)['catch'](function (deviceErr) {
                            return attemptTokenExchange(tempToken)['catch'](function (err) {
                                //If all normal attempts to long fail, we should try to use the masq
                                //token, if that fails - we should see a normal login
                                if (shouldAttemptMasquerade()) {
                                    return createOAuthSessionFromMasqueradeToken(getMasqueradeToken(), tempToken).then(function (oauthData) {
                    ...
                    // time.
                    var connectivitySvc = $injector.get('connectivityService');
        ...
        function refreshTokenWithDeviceVerifier(tempToken) {
            var deviceVerifier = OauthDataManager.get().deviceVerifier;
            if (deviceVerifier) {
                ...
                return authorizeDevice(deviceVerifier, tempToken).then(function (verifier) {
        ...
        function attemptTokenExchange(temp) {
            var shouldAttempt = shouldAttemptSso();
            if (shouldAttempt && shouldRejectAutoOauth()) {
                ...
                if (!isMobile) {
                    return tokenExchange(temp).then(function (verifier) {
                        return getToken(verifier, temp);
                    });
                } else {
        ...
        function refreshTokenWithAutoVerifier(tempToken) {
            ...
            return autoAuthorize(tempToken).then(function (verifier) {
                return getToken(verifier, tempToken);
            }).then(function () {
                // Autoauth success
                ...
                if (profileService.isSpecUOrBulkMDU()) {
                    return shouldRejectAutoOauth() ? $q.reject({
                    ...
                    return profileService.postAuth(true).then(function () {
                        // Show login screen for video customer not subscribed to IPTV package
                        return profileService.isIptvPackage().then(function (isIpOnly) {
                            return profileService.canUseTwctv().then(function () {
                                // has video capabilitiy
                                if (!isIpOnly) {
                                    return $q.reject('ipOnlyError');
                                }
                            })['catch'](function (err) {
                                // no video capabilitiy
                                if (err === 'ipOnlyError') {
                                    throw err;
                                } else {
                                    return true;
        ...
        function getToken(verifier, temp) {
            ...
            if (!_getTokenMutex) {
                ...
                _getTokenMutex = $http(ajaxOptions).then(function (response) {
                    ...
                    // Analytics
                    var errorResponse = getOAuthErrorCodeFromResponse(err);
                    $rootScope.$emit('Analytics:receivedOauthToken', {
                        success: false,
        ...
        function getOAuthDataFromResponse(oAuthResponse) {
            ...
            var oAuthString = oAuthResponse.data;
            // get keys and map them to preferred param name
            var dataPairs = httpUtil.getPairsFromQueryString(oAuthString);
            // Look for each known parameter key and get the value, assign it to dataMap
            return Object.keys(dataPairs).reduce(function (memo, key) {
        ...
        function getOAuthErrorCodeFromResponse(oAuthErrorCodeResponse) {
            ...
            if (oAuthErrorCodeResponse) {
                ...
                clientErrorMessage = 'object' === typeof oAuthErrorCodeResponse ? JSON.stringify(oAuthErrorCodeResponse) : oAuthErrorCodeResponse;
        ...
        function getTemporaryToken() {
            ...
            }).then(getOAuthDataFromResponse, function (err) {
                $rootScope.$emit('Analytics:loginStop', {
                    response: getOAuthErrorCodeFromResponse(err),
                    success: false
                });
        ...
        function getHeader(request) {
            ...
            paramsHeader = Object.keys(params).map(function (key) {
...
//# sourceMappingURL=../../maps-babel/ovpApp/oauth/oauth-service.js.map

...
(function () {
    ...
    loginEvents.$inject = ["$rootScope", "analyticsService", "$log"];
    angular.module('ovpApp.analytics.events.login', ['ovpApp.analytics.analyticsService']).factory('login', loginEvents).run(["login", function loadHandler(login) {
        return login;
    }]);
    ...
    function loginEvents($rootScope, analyticsService, $log) {
        ...
        function attachEventListeners() {
            try {
                $rootScope.$on('Analytics:loginStop', function (event, data) {
                    loginStop(data);
                });

                $rootScope.$on('Analytics:loginStart', function (event, data) {
                    loginStart(data);
                });

                $rootScope.$on('Analytics:logout', function (event, data) {
                    logout(data);
                });

                $rootScope.$on('Analytics:inVisitOauthRefresh', function (event, data) {
                    inVisitOauthRefresh(data);
                });
                $rootScope.$on('Analytics:receivedOauthToken', function (event, data) {
                    receivedOauthToken(data);
                });
                $rootScope.$on('Analytics:forcedLogin', function (event, data) {
                    forcedLogin(data);
                });
                $rootScope.$on('Analytics:captureAuthAttemptId', function (event, data) {
        ...
        function loginStop(data) {
            try {
                ...
                // If we have a response, emit error event.
                        ...
                        } else if (data.response.headers && data.response.headers('x-pi-auth-failure') && data.response.data) {
        ...
        return {
            ...
            expandOauthToken: expandOauthToken,
            loginStop: loginStop,
            logout: logout,
            inVisitOauthRefresh: inVisitOauthRefresh,
            getAsStringOrJSON: getAsStringOrJSON,
            captureAuthAttemptId: captureAuthAttemptId
        };
    }
})();
//# sourceMappingURL=../../../maps-babel/ovpApp/analytics/events/login.js.map

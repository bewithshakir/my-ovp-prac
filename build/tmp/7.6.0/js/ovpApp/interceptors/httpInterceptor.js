...
(function () {
    ...
    angular.module('ovpApp.interceptors.httpInterceptor', ['ovpApp.config']).config(httpInterceptor);
    ...
    function httpInterceptor($httpProvider) {
        $httpProvider.interceptors.push(['$q', '$injector', '$log', '$rootScope', function ($q, $injector, $log, $rootScope) {
            ...
            function processResponse(response) {
                ...
                if (!response || !response.config || !response.config.url || !response.status) {
                    return;
                }
                ...
                var statusCodeInt = parseInt(response.status, 10);
                ...
                var serviceResult = isSuccess ? 'success' : 'failure';
                ...
                var queryParameters = '';
                var queryIdx = response.config.url.indexOf('?');
                if (queryIdx > 0) {
                ...
                if (response.config.cache && ('GET' === response.config.method || 'JSONP' === response.config.method)) {

                    // Generate URL w/ parameters using same strategy as Angular $http.
                    var urlWithParams = response.config.url;
                    if (response.config.params) {
                        var serializedParams = response.config.paramSerializer(response.config.params);
                        if (serializedParams.length > 0) {
                            urlWithParams += (urlWithParams.indexOf('?') == -1 ? '?' : '&') + serializedParams;
                        }
                    }

                    var cachedEntityArray = response.config.cache === true || response.config.cache.get(urlWithParams);
                ...
                if (!isSuccess) {
                    ...
                    apiCallPayload.errorMessage = response.statusText + ': ' + JSON.stringify(response.data);
                }
...
//# sourceMappingURL=../../maps-babel/ovpApp/interceptors/httpInterceptor.js.map

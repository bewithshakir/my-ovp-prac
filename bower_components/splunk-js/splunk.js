...
var ThrottleService = (function(config) {
    ...
    function init(config) {
        reportFrequencyMs = parseInt(config.splunkControlParameters.reportFrequencyMs),
        maxBufferSizeRecords = parseInt(config.splunkControlParameters.maxBufferSizeRecords),
...
var splunk = (function() {
    ...
    var sequence = 0,
        timerPromise = null,
        heartbeatFrequencyMs = null, // set to null to initially start the timer
        loggingEnabled = null,
     ...
     */
    function sendMessage(message) { //TODO - check how throttle service will be injected?
        if (!loggingEnabled) {
            return;
        }
    ...
    function updateConfiguration(config) {
        ...
        heartbeatMessage.typeData.config.splunkPlayerStatusEnabled =
            config.getBool(config.splunkControlParameters.splunkPlayerStatusEnabled);
        heartbeatMessage.typeData.config.splunkHeartbeatEnabled =
            config.getBool(config.splunkControlParameters.splunkHeartbeatEnabled);
    ...
    */
    function sendPlayerStatus(statusDetails) {
        ...
        let playerStatusDetails = formatPlayerStatusDetails(statusDetails);
        let message = buildLogMessage(playerStatusDetails);
        message.level = 'INFO';
        sendMessage(message);
    }
    ...
    */
    ...
    function endTimer(name) {
        if (timers[name]) {
            let elapsed = Date.now() - timers[name];
            delete timers[name];
            sendStats({
                type: 'navigation',
                elapsedMS: elapsed
    ...
    */
    ...
    function sendError(exception, cause, source, config) {
        if (config.splunkControlParameters.splunkExceptionLoggingEnabled) {
            ...
            let message = buildLogMessage(errorMessage);
            message.level = 'ERROR';
            sendMessage(message);
        }
    }
    ...
    function sendStats(statMessage) {
        let message = buildLogMessage(statMessage);
        message.level = 'INFO';
        sendMessage(message);
    }

...
(function () {
    ...
    navigationEvents.$inject = ["$rootScope", "analyticsService", "$log", "analyticsEnums", "$window", "stringUtil", "navigationTimer", "config", "$q", "analyticsAssetHelper", "playerService", "analyticsMenuItemData", "analyticsPageViewData"];
    angular.module('ovpApp.analytics.events.navigation', ['ovpApp.config', 'ovpApp.analytics.analyticsService', 'ovpApp.legacy.stringUtil', 'ovpApp.analytics.events.navigationTimer', 'ovpApp.analytics.analyticsAssetHelper']).factory('navigation', navigationEvents).constant('analyticsMenuItemData', {
        'livetv': 'liveTv',
        'ovp.guide': 'guide',
    ...
    }).run(["navigation", function loadHandler(navigation) {
        return navigation;
    }]);

    /* @ngInject */
    function navigationEvents($rootScope, analyticsService, $log, analyticsEnums, $window, stringUtil, navigationTimer, config, $q, analyticsAssetHelper, playerService, analyticsMenuItemData, analyticsPageViewData) {
        ...
        function startNavigation(e, data) {
            try {
                ...
                if (angular.isDefined(data.transition.targetState) && angular.isDefined(data.transition.targetState().params()) && angular.isDefined(data.transition.targetState().params().analyticsAppTriggeredRouting)) {
                    appTriggeredRouting();
                }

                // Consume the triggeredByMemento, if it has been populated.
                var triggeredBy = triggeredByMemento || 'user';
                triggeredByMemento = null;

                // Retrieve any analytics for the current destination.
                var pageAnalytics = analyticsPageViewData[data.transition.to().name] || data.toState.analytics;
                ...
                var appSection = pageAnalytics.appSection;
                var pageName = null;
                ...
                var analyticSection = pageAnalytics;
                ...
                if (angular.isDefined(data.toParams)) {
                    var str = Object.keys(data.toParams).sort().map(function (key) {
                ...
                if (state.getCurrentPageName() === pageName && state.getCurrentAppSection() === appSection) {
                    if (analyticsService.isDebug()) {
                ...
                if (routingState && routingState.appSection === appSection && routingState.pageName === pageName && routingState.pageId === pageId) {
                    if (analyticsService.isDebug()) {
                ...
                timeoutTimer = navigationTimer.startTimer(routingState.appSection, routingState.pageName);
        ...
        function extractFilters(appSection, pageName, params) {
            ...
            if (appSection === 'curatedCatalog' && pageName === 'curatedVideoStore') {
                if (params.category && params.category.indexOf('&catName=') > 0) {

                    var queryParameters = stringUtil.extractSingleValueParametersAsObject(params.category);
        ...
        function partiallyRendered() /*e, data*/{
            try {
                ...
                if (!routingState) {
                    ...
                    if (analyticsService.isDebug()) {
                        $log.debug('Analytics: Ignoring partiallyRendered event because ' + 'not tracking a navigation event.');
                    }
                    return;
                }
        ...
        function pageChangeComplete() /*e, data*/{
            try {
                ...
                if (!routingState) {
                    ...
                    // search-input.
                    if (analyticsService.isDebug()) {
                        $log.debug('Analytics: Ignoring pageComplete event because ' + 'it has already been submitted.');
                    }
                    return;
                }
        ...
        function modalView(e, data) {
            ...
            try {
                ...
                var eventData = angular.copy(data);
        ...
        function flashWarning(e, data) {
            try {
                if (analyticsService.isDebug()) {
                    $log.debug('Analytics: flashWarning, currPage=' + state.getCurrentPageName(), data, routingState);
                }
        ...
        function selectContent(e, data) {
            try {
                if (analyticsService.isDebug()) {
                    $log.debug('Analytics: selectContent, page=' + state.getCurrentPageName(), data);
                }
                ...
                var pageName = state.getCurrentPageName();
                if ('curatedVideoStore' === pageName) {

                    // TVOD purchase
                    payload.elementStandardizedName = 'videoStore';
                } else if (['search', 'curatedCastAndCrew', 'curatedSports'].indexOf(pageName) >= 0 && data.asset && data.asset.resultDisplay) {
                ...
                var assetOrShow = data.asset || data.showFromGuide;
                ...
                payload.tmsGuideId = analyticsAssetHelper.getTmsGuideId(data.channel) || analyticsAssetHelper.getTmsGuideId(assetOrShow);
        ...
        function guideUpdateFilters(e, data) {
            try {
                ...
                // on the live tv page?
                var isMiniGuide = 'playerLiveTv' === state.getCurrentPageName();

                // Drop this event if the filter hasn't actually changed.
                var oldFilter = isMiniGuide ? state.getCurrentMiniGuideFilter() : state.getCurrentGuideFilter();
                var newFilter = data.filter ? data.filter.label || data.filter.name : null;
        ...
        function guideUpdateSort(e, data) {
            try {
                ...
                if (data.sort && data.sort.name) {
                    ...
                    data.elementStandardizedName = data.sort.id === 'networkAToZ' ? 'sortByAlphabetical' : 'sortByNumber';
                }
        ...
        function selectAction(e, data) {
            try {
                ...
                if ('DeriveFromOpType' === data.pageSectionName) {
                    var currPage = state.getCurrentPageName();
                ...
                if (data.nonNormalizedElementStdName) {
                    data.elementStandardizedName = analyticsAssetHelper.normalizeContentCategoryName(data.nonNormalizedElementStdName);
                }
                ...
                if (data.context === 'tvodFlow') {
                    ...
                    if (tvodFlow.purchaseStopSeen === true && data.ignoreIfPurchaseStopIsSeen === true) {
                        if (analyticsService.isDebug()) {
                    ...
                    if (data.asset) {
                        data.recordingOptions = analyticsAssetHelper.extractRecordingOptions(data.asset, data.cdvrSettings);
                    }
            ...
            try {
                ...
                if (data.error || data.errorCode) {
                    ...
                    if (!angular.isDefined(data.errorCode)) {
                        data.errorCode = 'dvr' === state.getLastPlaybackType() ? 'WCM-1603' : 'WCM-1001';
                    }
        ...
        function showPinDialog(e, data) {
            try {
                ...
                var currentPage = state.getCurrentPageName();
        ...
        function pinEntry(e, data) {
            try {
                ...
                var pinEntryContext = state.getPINEntryContext();
                ...
                var pinData = angular.copy(pinEntryContext);
        ...
        function tvodPurchaseStart() {
            try {
                ...
                var purchaseStartData = angular.extend({}, tvodFlow);
        ...
        function tvodPurchaseStop(e, data) {
            try {
                if (analyticsService.isDebug()) {
                    $log.debug('Analytics: tvodPurchaseStop', angular.copy(data), angular.copy(state.getTvodFlowState()));
                }
                ...
                // Build event data
                var purchaseData = angular.extend({}, tvodFlow, data);
        ...
        function rdvrPrioritization(evt, data) {
            try {
                ...
                var eventData = {
                    ...
                    operationType: evt.name === 'Analytics:rdvr-higher-priority' ? 'increasePriority' : 'decreasePriority'
                };
        ...
        function menuItemClick(e, menuItem) {
            ...
            try {
                ...
                var elementName = analyticsMenuItemData[menuItem.link];
                if (elementName) {
                    selectAction(null, {
        ...
        return {
            appTriggeredRouting: appTriggeredRouting,
            attachEventListeners: attachEventListeners,
            closePinDialog: closePinDialog,
            extractFilters: extractFilters,
            extractSorts: extractSorts,
            flashWarning: flashWarning,
            goToDesktopVersionClicked: goToDesktopVersionClicked,
            guideUpdateFilters: guideUpdateFilters,
            guideUpdateSort: guideUpdateSort,
            modalStart: modalStart,
            modalView: modalView,
            pageChangeComplete: pageChangeComplete,
            ...
            timerTimedout: timerTimedout,
            tvodPurchaseStart: tvodPurchaseStart,
            tvodPurchaseStop: tvodPurchaseStop,
            unpauseNavigation: unpauseNavigation
        };
    }
})();
//# sourceMappingURL=../../../maps-babel/ovpApp/analytics/events/navigation.js.map

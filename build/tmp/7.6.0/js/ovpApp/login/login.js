...
(function () {
    ...
    LoginController.$inject = ["$scope", "$window", "$q", "subViews", "loginService", "$log", "alert", "promiseTracker", "runOnce", "config", "messages", "$state", "profileService", "httpUtil", "ovpStorage", "storageKeys", "$rootScope", "$timeout", "LOGIN_ERROR_TEMPLATES", "$element", "$location", "connectivityService", "streamPlusService", "errorCodesService", "locationService"];
    loginForm.$inject = ["messages", "LOGIN_ERROR_TEMPLATES", "ovpStorage", "storageKeys", "$window", "OauthService", "$rootScope"];
    angular.module('ovpApp.login', ['ovpApp.login.loginService', 'ovpApp.components.alert', 'ovpApp.legacy.httpUtil', 'ovpApp.config', 'ovpApp.messages', 'ajoslin.promise-tracker', 'ovpApp.oauth', 'ovpApp.services.profileService', 'ovpApp.services.streamPlusService', 'ovpApp.services.runOnceService', 'ovpApp.services.ovpStorage', 'ovpApp.services.locationService', 'ovpApp.directives.focus', 'ovpApp.services.errorCodes', 'ui.router', 'ngAria', 'ngMessages', 'ngCookies']).constant('subViews', {
        PARENTAL: 'parental',
        HELP: 'help',
        FORM: 'form',
        ERROR: 'error'
    ...
    }).controller('LoginController', LoginController).directive('ovpLoginForm', loginForm);

    /* @ngInject */

    function LoginController($scope, $window, $q, subViews, loginService, $log, alert, promiseTracker, runOnce, config, messages, $state, profileService, httpUtil, ovpStorage, storageKeys, $rootScope, $timeout, LOGIN_ERROR_TEMPLATES, $element, $location, connectivityService, streamPlusService, errorCodesService, locationService) {
        ...
        $scope.enterSite = function () {
            ...
            profileService.canUseTwctv().then(function () {
                $scope.showParentalControlWarning().then(function () {
                    var storedLink = ovpStorage.getItem(storageKeys.appLink);
                    if (storedLink) {
                        ...
                        profileService.canWatchLive().then(function (isEnabled) {
                            $scope.success = true;
                            if (isEnabled) {
                                $state.go('ovp.livetv');
                            } else {
                                profileService.getFirstAvailableState().then(function (state) {
                                    $state.go(state);
                                });
                            }
                        }, function (error) {
                ...
                streamPlusService.isStreamPlusEligible(true).then(function () {
        ...
        $scope.showParentalControlWarning = function () {
            if (profileService.isSpecUOrBulkMDU()) {
                ...
                loginService.hasAcceptedParentalControlsWarning().then(function (hasAccepted) {
                    if (hasAccepted) {
                        $scope.parentalControlWarningDeferred.resolve();
                    } else {
        ...
        $scope.authenticate = function () {
            if (!connectivityService.isOnline()) {
                ...
                var authenticationPromise = $q.when(loginService.authenticate($scope.user.username, $scope.user.password, $scope.user.rememberme)),
    ...
    /* @ngInject */
    function loginForm(messages, LOGIN_ERROR_TEMPLATES, ovpStorage, storageKeys, $window, OauthService, $rootScope) {
        return {
            restrict: 'E',
            templateUrl: '/js/ovpApp/login/loginForm.html',
            link: function link($scope) {
                var oauthError = OauthService.getOAuthError();
                ...
                $scope.$watch('errorCode', function (nv, ov) {
                    if (nv) {
                        ...
                        Object.keys(LOGIN_ERROR_TEMPLATES).forEach(function (key) {
...
//# sourceMappingURL=../../maps-babel/ovpApp/login/login.js.map

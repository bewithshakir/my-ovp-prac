...
(function () {
    'use strict';
    epgsService.$inject = ["$http", "$q", "ServiceError", "config", "stbService", "profileService"];
    realEpgsService.$inject = ["$http", "$q", "ServiceError", "config", "stbService", "profileService"];
    angular.module('ovpApp.services.epgsService', ['ovpApp.config', 'ovpApp.components.error.ServiceError', 'ovpApp.services.stbService']).factory('epgsService', epgsService).factory('realEpgsService', realEpgsService);
    ...
    function epgsService($http, $q, ServiceError, config, stbService, profileService) {
        ...
        var service = {
            ...
            getChannelByMystroSvcId: getChannelByMystroSvcId,
            getChannelByNcsSvcId: getChannelByNcsSvcId,
        ...
        function getChannels() {
            if (channelsPromise === null) {
                channelsPromise = $q.all([profileService.isIpOnlyEnabled(), stbService.getHeadend()]).then(function (params) {
                    var ipOnlyEnabled = params[0];
                    var headend = params[1];
                    var queryParams = {};
                    var requestUrl = piHost + smartTv;
                    ...
                    return $http.get(requestUrl, queryParams).then(function (response) {
                        ...
                        if (!ipOnlyEnabled) {
                            angular.forEach(response.data.channels, function (channel, index) {
                                ...
                                if (channel && channel.mystroServiceId !== undefined && channel.channelNumber !== undefined) {
        ...
        function enrichChannels(channels) {
            ...
            channels.forEach(function (channel) {
                ...
                if (channel.channels && channel.channels.length) {
                    ...
                    channel.channels.forEach(function (channelNumber, channelIndex) {
            ...
            (_channels = channels).push.apply(_channels, duplicates);
        ...
        function enrichChannelData(channel, channelNumber) {
            return angular.extend(channel, {
                ...
                fullLogoUrl: (channel.logoUrl && channel.logoUrl.startsWith('/') ? config.piHost + channel.logoUrl : channel.logoUrl) + '?width=50&sourceType=colorhybrid'
            });
        }
...
//# sourceMappingURL=../../maps-babel/ovpApp/services/epgs-service.js.map

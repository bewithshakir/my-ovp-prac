...
(function () {
    ...
    Analytics.$inject = ["config", "httpUtil", "version", "deviceid", "$window", "analyticsService", "playback", "login", "error", "apiCall", "analyticsDvr", "analyticsAssetHelper", "navigation", "navigationTimer", "analyticsEnums", "search", "sessionAnalytics", "productActionAnalytics", "windowAnalytics", "$log", "stringUtil", "rx"];
    loadAnalytics.$inject = ["Analytics"];
    angular.module('ovpApp.analytics', ['ovpApp.config', 'ovpApp.legacy.httpUtil', 'ovpApp.legacy.stringUtil', 'ovpApp.service.version', 'ovpApp.legacy.deviceid', 'ovpApp.analytics.events.playback', 'ovpApp.analytics.events.login', 'ovpApp.analytics.events.error', 'ovpApp.analytics.events.apiCall', 'ovpApp.analytics.events.navigation', 'ovpApp.analytics.events.navigationTimer', 'ovpApp.analytics.events.search', 'ovpApp.analytics.events.sessionAnalytics', 'ovpApp.analytics.events.productActionAnalytics', 'ovpApp.analytics.events.windowAnalytics', 'ovpApp.analytics.events.dvr', 'ovpApp.analytics.analyticsService', 'ovpApp.analytics.analyticsEnums', 'ovpApp.analytics.analyticsAssetHelper', 'rx']).factory('Analytics', Analytics).run(loadAnalytics);

    /* @ngInject */
    function Analytics(config, httpUtil, version, deviceid, $window, analyticsService, playback, login, error, apiCall, analyticsDvr, analyticsAssetHelper, navigation, navigationTimer, analyticsEnums, search, sessionAnalytics, productActionAnalytics, windowAnalytics, $log, stringUtil, rx) {
        var queryPairs = httpUtil.getPairsFromQueryString();
        var previousVisitId = null;
        ...
        function initialize() {
            ...
            try {
                ...
                if (config.analytics.venona.enabled) {
                    var fields = setSessionFields();
                    var options = { timestamp: Date.now() };
        ...
        function generateVisitId() {
            var visitId = queryPairs.analyticsVisitId || stringUtil.guid();
            var previousId = null;
...
//# sourceMappingURL=../../maps-babel/ovpApp/analytics/analytics.js.map

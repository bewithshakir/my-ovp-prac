...
(function () {
    ...
    ovpClick.$inject = ["$parse"];
    ovpKeydown.$inject = ["$timeout", "keyMap"];
    globalKeydown.$inject = ["$document", "rx"];
    angular.module('ovpApp.directives.keydown', ['rx']).factory('globalKeydown', globalKeydown).directive('ovpKeydown', ovpKeydown).directive('ovpClick', ovpClick).constant('keyMap', {
        3: 'break',
        8: 'delete',
        9: 'tab',
        12: 'clear',
    ...
    function ovpKeydown($timeout, keyMap) {
        ...
        function link(scope, element, attrs) {
            var params = {};
            $timeout(function () {
                angular.forEach(scope.ovpKeydown, function (val, key) {
                    var keys = key.split('+').map(function (a) {
            ...
            angular.element(element).on('keydown', function (event) {
                ...
                if (angular.isFunction(handler) && !event.metaKey && !event.shiftKey && !event.ctrlKey && !event.altKey) {
                    handler(event);
                    prevent(event, attrs);
                } else if (angular.isObject(handler) && angular.isFunction(handler.callback) && event[handler.key + 'Key'] === true) {
    ...
    function globalKeydown($document, rx) {
        var observable = rx.Observable.fromEvent($document[0].body, 'keydown');
        return { observable: observable };
    }
})();
//# sourceMappingURL=../../maps-babel/ovpApp/directives/ovp-keydown.js.map

...
(function () {
    'use strict';
    oauthInterceptor.$inject = ["$httpProvider"];
    angular.module('ovpApp.interceptors.oauth', ['ovpApp.config']).config(oauthInterceptor);
        ...
        $httpProvider.interceptors.push(['$q', '$injector', function ($q, $injector) {
            ...
            return {
                request: function request(config) {
                    ...
                    if (config.url.indexOf(appConfig.oAuth.tokenExchange) === -1 && (config.url.indexOf(appConfig.piHost) === 0 || config.url.indexOf(appConfig.authNeededTrackingDomain) >= 0)) {
                        if (!config.bypassRefresh) {
                            //Most requests will pass through this block if they dont have anything to do
                            //with authentication.
                            return OauthService.updateToken().then(function () {
                                return appendHeaders(config, OauthService);
                            }, function (err) {
                                return $q.reject('User is not logged in, retry after login ' + config.url + '[' + err + ']');
                            });
                        } else {
                            return appendHeaders(config, OauthService);
                        }
            ...
            function processResponseError(response) {
                if (response.config && response.status === 401 && !response.config.bypassRefresh && ( //Do not attempt to refresh if a 400+ response might be expected
                response.config.retryCount === undefined || response.config.retryCount < MAX_RETRY_COUNT)) {

                    var OauthService = $injector.get('OauthService');
                    return OauthService.updateToken(true).then(function () {
...
//# sourceMappingURL=../../maps-babel/ovpApp/interceptors/oauthInterceptor.js.map

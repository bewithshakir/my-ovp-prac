...
(function () {
     ...
     */
    delegateFactory.$inject = ["$log"];
    angular.module('ovpApp.dataDelegate', ['dataDelegate', 'ovpApp.config', 'ovpApp.messages', 'ovpApp.services.entitlementsService', 'ovpApp.services.parentalControlsService', 'ovpApp.services.bookmark', 'ovpApp.services.dateFormat', 'ovpApp.services.channel', 'ovpApp.services.errorCodes', 'ovpApp.product.service', 'ovpApp.filters.titleCase', 'ui.router']).factory('delegateFactory', delegateFactory);
    ...
    function delegateFactory($log) {
        ...
        var service = {
            registerDelegateDefinition: registerDelegateDefinition,
        ...
        function registerDelegateDefinition(definition, idFxn) {
            registrations[registrations.length] = { definition: definition, idFxn: idFxn };
        }
        ...
        function createInstance(asset) {
            var matches = registrations.filter(function (r) {
                return r.idFxn(asset);
            });
            if (matches.length > 1) {
...
//# sourceMappingURL=../../maps-babel/ovpApp/data-delegates/delegate-factory.js.map
